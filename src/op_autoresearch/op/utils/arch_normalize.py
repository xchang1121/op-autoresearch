...
short arch token used by OP_AUTORESEARCH while keeping future cards data-driven.
"""
...
def normalize_cuda_arch_name(name: str) -> Optional[str]:
    """Return OP_AUTORESEARCH's short CUDA arch token from a vendor model string.
...
def normalize_cpu_arch_name(name: Optional[str] = None) -> Optional[str]:
    """Normalize platform CPU arch names into OP_AUTORESEARCH's canonical tokens."""
