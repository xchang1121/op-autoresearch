...
_OP_AUTORESEARCH_CATLASS_ROOT_BLOCK = f"""# Resolved by op_autoresearch (task.yaml catlass.root / -DCATLASS_ROOT / ENV{{CATLASS_ROOT}} / <op_autoresearch-root>/thirdparty/catlass)
...
elseif(DEFINED ENV{{CATLASS_ROOT}} AND NOT "$ENV{{CATLASS_ROOT}}" STREQUAL "")
  ...
  message(FATAL_ERROR "CATLASS_ROOT not set. Pass -DCATLASS_ROOT, export CATLASS_ROOT, or install catlass at <op_autoresearch-root>/thirdparty/catlass via `bash scripts/download_catlass.sh`.")
endif()
...
def _bundled_catlass_root() -> Optional[str]:
    ...
    # __file__ = .../src/op_autoresearch/op/utils/catlass_paths.py
    # repository root = four parents above the installed source package
