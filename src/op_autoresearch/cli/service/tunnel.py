...
def tunnel_stop_silent(port: int, ssh_alias: str = "") -> None:
    ...
    if pid:
        try:
            ...
            print(f"[op-autoresearch] tunnel stop failed: {e}", file=sys.stderr)
    try:
        pid_path.unlink()
...
def tunnel_start(ssh_alias: str, port: int) -> int:
    ...
    if os.name == "posix":
        ...
        if rc != 0:
            print(f"[op-autoresearch] ssh -L exit rc={rc}", file=sys.stderr)
    else:
        ...
        try:
            ...
            print(f"[op-autoresearch] ssh -L spawn failed: {e}", file=sys.stderr)
            return 0
