...
async def register_remote_worker(backend: str, arch: str, worker_url: Optional[str] = None, capacity: Optional[int] = None, tags: Set[str] = None,
    ...
    如果未提供 worker_url，将从环境变量 OP_AUTORESEARCH_WORKER_URL 读取。
    如果未提供 capacity，将自动从 remote worker 的 status 接口查询实际设备数量。
    
    Args:
        backend: 后端类型 (cuda/ascend/cpu)
        arch: 硬件架构 (a100/ascend910b4等)
        worker_url: Remote Worker Service 的 URL（可选，默认从环境变量 OP_AUTORESEARCH_WORKER_URL 读取）
    ...
    if worker_url is None:
        ...
        if worker_url is None:
            raise ValueError(
                "worker_url 未提供且环境变量 OP_AUTORESEARCH_WORKER_URL/OP_AUTORESEARCH_WORKER_URL 未设置。\n"
...
async def register_worker(
    ...
    *,
    ...
    优先级：
      1. 若提供 worker_url 或设置 OP_AUTORESEARCH_WORKER_URL，则注册 RemoteWorker
