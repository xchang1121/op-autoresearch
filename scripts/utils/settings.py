...
# Worker port / report timing
# Accessor, now offline - op-autoresearch Full from ``cli/service/worker_config.WorkerConfig``
# There is no script to directly call a field that reads yaml, workspace.*. If needed
# Read in WA script. * Please change to `fram op_autoresearch.cli.service.worker_config
# Report WorkerConfig; cfg = WorkerConfig.load() ' `Wide a single source of fact.
